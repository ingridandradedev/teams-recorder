...
class TeamsFeedbackService:
    """Serviço especializado para análise de feedback 1:1 com PNL integrado à gravação do Teams"""
    ...
    def create_feedback_session(self, session_id: str, teams_url: Optional[str] = None, recording_id: Optional[str] = None) -> FeedbackSession:
        """Cria uma nova sessão de feedback com contexto integrada à gravação do Teams"""
        now = datetime.now()
        ...
        logger.info(f"Nova sessão de feedback PNL com Teams criada: {session_id} | Teams: {teams_url}")
    ...
    def resume_feedback_session(self, session_id: str) -> bool:
        """Retoma sessão de feedback"""
        if session_id in self.active_sessions:
            self.active_sessions[session_id].status = "active"
            logger.info(f"Sessão de feedback retomada: {session_id}")
            return True
        return False
    ...
    def _create_contextual_prompt(self, context: ConversationContext, chunk_number: int) -> str:
        """Cria prompt com contexto acumulado da conversa integrada ao Teams"""
        ...
        # Adiciona contexto específico do Teams se disponível
        teams_context = ""
        if context.teams_url:
            teams_context = f"\n**CONTEXTO DA REUNIÃO TEAMS**: Esta é uma reunião do Microsoft Teams gravada automaticamente."
        
        base_prompt = f"""
        Você é um especialista em Programação Neurolinguística (PNL) analisando uma REUNIÃO DE FEEDBACK 1:1 GRAVADA NO TEAMS.
        
        **IMPORTANTE**: Esta é uma análise CONTEXTUAL de áudio capturado durante gravação do Teams. Você está recebendo um segmento de áudio da conversa em andamento (aproximadamente aos {tempo_atual} minutos).
        {teams_context}
        ...
        **PADRÕES PNL IDENTIFICADOS ANTERIORMENTE:**
        {", ".join(context.padroes_identificados) if context.padroes_identificados else "Nenhum padrão identificado ainda"}
        
        **EVOLUÇÃO EMOCIONAL OBSERVADA:**
        {", ".join(context.evolucao_emocional) if context.evolucao_emocional else "Observando estados iniciais"}
        
        **TEMAS RECORRENTES:**
        {", ".join(context.temas_recorrentes) if context.temas_recorrentes else "Aguardando temas recorrentes"}
    ...
    async def process_feedback_audio_with_context(
        ...
        """
        Processa áudio de feedback 1:1 com contexto acumulativo integrado ao Teams
        ...
        try:
            ...
            # Verifica se sessão está pausada
            if session.status == "paused":
                logger.info(f"Sessão pausada, ignorando áudio: {session_id}")
                return None
            ...
            logger.info(f"Processando áudio da sessão Teams {session_id} (tempo ~{tempo_atual}): {len(audio_bytes)} bytes")
            ...
            logger.info(f"Resposta contextual de feedback PNL Teams recebida (tempo ~{tempo_atual})")
            ...
            try:
                ...
                logger.info(f"Contexto atualizado para sessão Teams {session_id}: {session.total_chunks} segmentos, {len(session.context.padroes_identificados)} padrões")
                
                return new_analysis
                
            except json.JSONDecodeError as e:
                logger.error(f"Erro ao fazer parse do JSON de feedback contextual Teams: {e}")
                logger.error(f"Resposta recebida: {response.text}")
                return None
                
        except Exception as e:
            logger.error(f"Erro ao processar áudio contextual de feedback Teams: {str(e)}")
            return None
    ...
    def list_all_sessions(self) -> Dict[str, dict]:
        """Lista todas as sessões ativas com resumo"""
        sessions_summary = {}
        for session_id, session in self.active_sessions.items():
            sessions_summary[session_id] = {
                ...
                "teams_url": session.teams_url,
                "recording_id": session.recording_id,
                "patterns_count": len(session.context.padroes_identificados),
                "themes_count": len(session.context.temas_recorrentes)
            }
